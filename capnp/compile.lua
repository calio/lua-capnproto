...
local function get_naming_func(name)
...
-- TODO this is global
local function get_schema_text(file)
...
-- TODO this is global
local function comp_header(res, nodes)
    dbg("compile_headers")
    insert(res, format([[
...
local function get_name(display_name)
...
local function get_size(type_name)
...
local function _set_field_default(nodes, field, slot)
    local default
...
local function _set_field_type(field, slot, nodes)
    local type_name
    if field.group then
...
local function comp_field(res, nodes, field)
    dbg("comp_field")
...
local function comp_parse_struct_data(res, nodes, struct, fields, size, name)
    insert(res, format([[
...
local function comp_parse(res, name)
    insert(res, format([[
...
local function comp_serialize(res, name)
    insert(res, format([[
...
local function comp_flat_serialize(res, nodes, struct, fields, size, name)
...
-- @param nodes
-- @param is_group
local function comp_calc_size(res, fields, size, name, nodes, is_group)
    dbgf("comp_calc_size")
    if is_group then
        size = 0
    end
...
local function comp_fields(res, nodes, node, struct)
    insert(res, [[
...
local function comp_struct(res, nodes, node, struct, name)
...
local function comp_enum(res, nodes, enum, name, enum_naming_func)
...
local function comp_node(res, nodes, node, name)
...
--          name = "struct.capnp" } } },
local function comp_file(res, nodes, file)
    dbg("comp_file")
    local id = file.id
...
local function check_import(files, import)
    local id = import.id
...
local function comp_import(res, nodes, import)
    local id = import.id
